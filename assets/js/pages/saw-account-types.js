...
(function($) {
    ...
    const AccountTypes = {
        ...
        loadAccountType: function(id) {
            $.ajax({
                url: (window.sawAccountTypesData && sawAccountTypesData.ajaxUrl) || window.ajaxurl || '/wp-admin/admin-ajax.php',
                type: 'POST',
                data: {
                    action: 'saw_account_types_list',
                    nonce: (window.sawAccountTypesData && sawAccountTypesData.nonce) || $('#account-type-form input[name="nonce"]').val(),
                    page: 1,
                    per_page: 1,
                    search: '',
                    filters: { id: id }
                ...
                success: function(response) {
                    if (response && response.success && response.data && response.data.items && response.data.items.length > 0) {
                        ...
                        console.warn('No data for account type id=', id, response);
                    }
                },
                error: function() {
        ...
        populateForm: function(data) {
            ...
            $('#is_active').prop('checked', String(data.is_active) === '1');
        },
        ...
        submitForm: function($form) {
            ...
            // Bezpečný fallback: nonce z lokalizace NEBO z hidden inputu ve formu
            const nonce =
                (window.sawAccountTypesData && sawAccountTypesData.nonce) ||
                $form.find('input[name="nonce"]').val() ||
                '';

            const ajaxUrl =
                (window.sawAccountTypesData && sawAccountTypesData.ajaxUrl) ||
                window.ajaxurl ||
                '/wp-admin/admin-ajax.php';

            const formData = $form.serialize() +
                '&action=saw_account_type_save' +
                ...
                success: function(response) {
                    if (response && response.success) {
                        if (window.SAWNotifications) {
                            window.SAWNotifications.success(response.data.message || 'Uloženo');
                        }
                        setTimeout(function() {
                            // Přesměrování zpět na list (funguje i v SPA režimu)
                            if (window.SAW_Navigation) {
                                SAW_Navigation.navigateTo('/admin/settings/account-types/');
                            } else {
                        ...
                        if (window.SAWNotifications) {
                            window.SAWNotifications.error(msg);
                        } else {
                            alert('Chyba: ' + msg);
                        }
        ...
        populateDetailModal: function(data) {
            ...
            const statusHtml = String(data.is_active) === '1'
