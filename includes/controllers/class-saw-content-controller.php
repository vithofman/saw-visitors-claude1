<?php
/**
 * SAW Content Management Controller - FIXED
...
class SAW_Content_Controller {
    ...
    public function index() {
        ...
        // CRITICAL: Pass $this (controller) to template
        $controller = $this;
        
        // Render template
        ob_start();
        include SAW_VISITORS_PLUGIN_DIR . 'templates/pages/admin/content.php';
     ...
     * Save material
     */
    private function save_material($type, $language, $file_url = '', $filename = '', $storage_type = 'file', $wysiwyg_content = '') {
        global $wpdb;
        
        $existing = $wpdb->get_row($wpdb->prepare(
            "SELECT id FROM {$wpdb->prefix}saw_materials 
            WHERE customer_id = %d AND title = %s AND language = %s",
            $this->customer_id, $type, $language
        ));
        ...
        if ($storage_type === 'url' || $storage_type === 'file') {
            $data['file_url'] = $file_url;
            $data['filename'] = $filename;
        } elseif ($storage_type === 'wysiwyg') {
        ...
        if ($existing) {
            ...
            return $wpdb->insert($wpdb->prefix . 'saw_materials', $data);
        }
    }
