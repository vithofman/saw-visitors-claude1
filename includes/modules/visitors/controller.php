...
class SAW_Module_Visitors_Controller extends SAW_Base_Controller 
{
    ...
    protected function format_detail_data($item) {
        ...
        // Load change history for this visitor
        if (!empty($item['id']) && class_exists('SAW_Audit')) {
            try {
                $entity_type = $this->config['entity'] ?? 'visitors';
                ...
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log('[SAW Audit] Failed to load change history for visitors: ' . $e->getMessage());
                }
            }
        }
