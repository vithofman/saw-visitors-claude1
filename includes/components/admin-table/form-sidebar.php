...
<div class="sa-sidebar sa-sidebar--active saw-sidebar<?php echo $is_nested ? ' saw-sidebar--nested' : ''; ?>" data-mode="<?php echo $is_edit ? 'edit' : 'create'; ?>" data-entity="<?php echo esc_attr($entity); ?>" data-module="<?php echo esc_attr($entity); ?>" data-is-nested="<?php echo $is_nested ? '1' : '0'; ?>" <?php if ($is_edit && !empty($item['id'])): ?>data-current-id="<?php echo intval($item['id']); ?>"<?php endif; ?>>
    <div class="sa-sidebar-header saw-sidebar-header" style="position: relative;">
        ...
        <?php if ($is_nested): ?>
            <!-- Nested sidebar: button only, no redirect -->
            <button type="button" class="sa-sidebar-close saw-sidebar-close" title="<?php echo esc_attr($tr('btn_close', 'Zavřít')); ?>" style="position: absolute; right: 16px; top: 50%; transform: translateY(-50%); background: none; border: none; cursor: pointer; padding: 8px; border-radius: 6px;">
        ...
        <?php else: ?>
            <!-- Normal sidebar: anchor with navigation -->
            <a href="<?php echo esc_url($close_url); ?>" class="sa-sidebar-close saw-sidebar-close" title="<?php echo esc_attr($tr('btn_close', 'Zavřít')); ?>" style="position: absolute; right: 16px; top: 50%; transform: translateY(-50%);">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <line x1="18" y1="6" x2="6" y2="18"></line>
                    <line x1="6" y1="6" x2="18" y2="18"></line>
                </svg>
    ...
    <?php 
    // ============================================
    // FAB SAVE BUTTON (form mode only)
    // Replaces footer save button with floating action
    // ============================================
    ?>
    <div class="sa-sidebar-fab-container sa-sidebar-fab-container--form">
        <button type="submit" 
                form="saw-<?php echo esc_attr($entity); ?>-form" 
                class="sa-sidebar-fab sa-sidebar-fab--save"
                title="<?php echo esc_attr($is_edit ? $tr('btn_save', 'Uložit změny') : $tr('btn_create', 'Vytvořit')); ?>">
