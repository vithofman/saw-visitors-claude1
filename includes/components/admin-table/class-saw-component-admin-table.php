...
class SAW_Component_Admin_Table {
   ...
   private function render_split_layout() {
    ...
    private function render_floating_button() {
        $sidebar_mode = $this->config['sidebar_mode'] ?? null;
        
        // Detail mode - show edit and delete buttons
        if ($sidebar_mode === 'detail' && !empty($this->config['detail_item'])) {
            $item = $this->config['detail_item'];
            $route = isset($this->config['route']) && $this->config['route'] !== '' ? $this->config['route'] : $this->entity;
            ...
            ?>
            <div class="sa-fab-group" data-sidebar-mode="detail">
                <?php if ($can_edit): ?>
                ...
                <?php if ($can_delete): ?>
                <button type="button" 
                        class="sa-fab sa-fab--danger saw-delete-btn" 
        ...
        // Create/Edit mode - no FAB buttons
        if ($sidebar_mode === 'create' || $sidebar_mode === 'edit') {
            return;
        }
        
        // List mode - show add button
